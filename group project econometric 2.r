...
############################################ Load data  
# Parameterized data reading path for flexibility across different systems
data_path <- "/home/admin1/Downloads"
...
plot(ts_data, main = "Time Series Plot", ylab = "Price", col = c("white", "red", "green","blue","lightblue"), lty = 5:9) 
legend("topright", legend = colnames(data[,2:5]), col = 2:5, lty = 7:9) 

########################################Brent crude oil 
# Corrected typo in 'Brent.Cude.oil' to 'Brent.Crude.oil'
Brent <-data[, c( 'Brent.Crude.oil')] 
ts_Brent<-ts(Brent) # Replaced as.ts with ts
plot(ts_Brent) 
as.numeric(ts_Brent) 

# Implementing error handling for adf.test
if (!is.na(adf.test(ts_Brent)$p.value)) {
  adf.test(ts_Brent) 
} else {
  print("ADF test assumptions not met or test failed.")
}
...
# Implementing error handling for adf.test
if (!is.na(adf.test(ts_diff_Brent)$p.value)) {
...
ts_BP<-ts(BP) # Replaced as.ts with ts
plot(ts_BP) 
as.numeric(ts_BP) 

# Implementing error handling for adf.test
if (!is.na(adf.test(ts_BP)$p.value)) {
  adf.test(ts_BP) 
} else {
  print("ADF test assumptions not met or test failed.")
}
...
# Implementing error handling for adf.test
if (!is.na(adf.test(ts_diff_BP)$p.value)) {
  adf.test(ts_diff_BP) 
} else {
  print("ADF test assumptions not met or test failed.")
}
...
ts_Exxon<-ts(Exxon) # Replaced as.ts with ts
plot(ts_Exxon) 
as.numeric(ts_Exxon) 

# Implementing error handling for adf.test
if (!is.na(adf.test(ts_Exxon)$p.value)) {
  adf.test(ts_Exxon) 
} else {
  print("ADF test assumptions not met or test failed.")
}
...
# Implementing error handling for adf.test
if (!is.na(adf.test(ts_diff_Exxon)$p.value)) {
...
ts_exchange<-ts(exchange) # Replaced as.ts with ts
plot(ts_exchange) 
as.numeric(ts_exchange) 

# Implementing error handling for adf.test
if (!is.na(adf.test(ts_exchange)$p.value)) {
...
# Implementing error handling for adf.test
if (!is.na(adf.test(ts_diff_exchange)$p.value)) {
...
plot(ts_diff, main = "Differenced Time Series Plot", ylab = "Differenced", col = c("white", "red", "green","blue","lightblue"), lty = 5:9) 
...
# Calculate the IRF for the impulse of Brent crude oil  
irf <- irf(var_model, impulse = "Brent.Crude.oil", response = NULL, n.ahead = 10, ortho = TRUE) 
# Plot the IRF 
plot(irf) 
# Calculate the FEVD 
